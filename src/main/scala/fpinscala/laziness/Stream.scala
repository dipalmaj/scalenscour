...
sealed trait Stream[+A] {
  ...
  def map[B](p: A => B): Stream[B] =
    foldRight(Stream[B]())((a, b) => cons(p(a), b))

  // todo more complicted test and fix
  def flatMap[B](p: A => Stream[B]): Stream[B] =
    foldRight(Stream[B]())((a, b) => p(a) match {
      case Cons(h, _) => cons(h(), b)
      case _ => empty
    })

  def append[C >: A](n: C): Stream[C] =
    foldRight(Stream[C]())((a, b) => b match {
      case Empty => cons(a, cons(n, Empty))
      case _ => cons(a, b)
    })
  ...
  def fib: Stream[Int] = unfold((0, 1))(up =>
    Some(up._1, (up._2, up._1 + up._2))
  )
  ...
  def zipWith[B](s2: Stream[B]): Stream[(A, B)] =
    unfold((this, s2))(sm => (sm._1, sm._2) match {
  ...
  // TODO fix me
  def scanRight[C >: A](z: C)(f: (C, => C) => C): Stream[C] =
    unfold(this.tails)({
